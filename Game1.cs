            ...
            if (ErrorTimeOut-- < 0)
            {
                if (DoesImagesInListHaveMoreThan1Images())
                {
                    ...
                    OutputText = "Click on the image/file icon to add or change the image slot.\n" +
                                 ...
                                 "spectral data, make sure each image aligned up with the proper\n" +
                                 "image slot.";
                ErrorTimeOut = -1;
            }
            if (findingPoint)
            ...
            spriteBatch.Begin();
                ...
                for (int i = imagesInList.Length + ((cameraImagesAreFrom == 0)? imagesInList2.Length: 0); i > 0; i--)
                {
                    ...
                    if (hoveringOver != i)
                    {
                        ...
                        {
                            spriteBatch.Draw((imagesInList2[(i - 9)] == null) ? BlankImage : imagesInList2[(i - 9)], new Microsoft.Xna.Framework.Rectangle(25 + (i - 9) * openStackValuePer, 25 + openStackValuePer*256/100, 256, 256), Microsoft.Xna.Framework.Color.White);
                        }
                    }
                }
                if (hoveringOver != 0)
